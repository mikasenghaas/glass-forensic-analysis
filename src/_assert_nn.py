...
def main():
    ...
    for i, dataset in enumerate(data.keys()):
        ...
        for j in range(len(epochs)):
            clf = NeuralNetworkClassifier(
                    layers = [DenseLayer(n_in=2, n_out=5, activation='relu', name='fc1')],
                    loss='cross_entropy', 
                    name=f'Simple NN'
                    )

            clf.fit(X, y, epochs=epochs[j], lr=0.01, batch_size=10, verbose=1)
    ...
    if SAVE:
        fig.savefig(f'{SAVEPATH}/custom_nn_tree.pdf')
